...
| Agent Release Bundle Pattern | Agent behavior is the emergent product of 5+ independently-versioned components (prompt, model, tool manifest, retrieval config, validator, memory config). Each change is a de-facto new release, but most teams track none of them. The discipline of treating agent deployment as an atomic, versioned bundle with behavioral canary gates and bundle-scoped rollback is the missing release engineering layer for AI. | I-056 | Extends s101 (deterministic sessions), s209 (observability), s222 (trajectory replay). Model version rollback is provider-dependent and cannot always be guaranteed. |
