...
|----------|---------|----------|-----------|
| 2026-07-03 | I-046 | WRITTEN — S-462 | Agentic Prompt Caching: Cache-Aware Agent Loop Design — gap: S-08 covers static prompt caching API basics but no entry addresses agent-loop-level cache architecture, cache-boundary design, three-layer stratification, or the cost dynamics of cache-break events in multi-turn sessions. arXiv:2601.06007 (Jan 2026) confirmed 41-80% cost reduction for agentic tasks with proper strategy — a gap worth documenting as a first-class pattern. System Prompt Only strategy is the safe default (40-50% savings, zero cache-break risk); Hierarchical is the optimization target (60-75%). Composite 9.20. Chosen over: Consensus Mechanisms for Agent Teams (too organizational/voting-focused, not enough technical depth for a stacks entry), Memory Consolidation (covered by I-041/S-447). |
