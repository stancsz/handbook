...
|---------|-------------|---------------------|-------|
| Poisoned Memory Persists Across Sessions | Web agents that observe malicious content and store it as memory create persistent vulnerabilities that survive session boundaries. eTAMP (Environment-injected Trajectory-based Agent Memory Poisoning) achieves up to 32.5% attack success with zero direct memory access — the attacker only needs to control a webpage the agent visits. Environmental stress (high task load, tool failures, ambiguous goals) amplifies susceptibility by 40–60%. Defenses: provenance-tagged memory with source-weighted retrieval, domain-scoped memory entries, short TTL on web-observed facts, and behavioral drift detection. | I-070 | arXiv:2604.02623 (Zou et al., April 2026); confirmed cross-site activation — poison spills across sessions even without shared memory. |
